...
def alerts():
    ...
    rows = r.json()
    alerts = [row for row in rows if row.get("motion") or row.get("gas") or row.get("alarm")]
    return alerts[:20]
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .live-dot { width:10px; height:10px; background:#22c55e; border-radius:50%; display:inline-block; margin-right:6px; animation: blink 1s infinite; }
...
<div class="header">
  ...
  <div style="text-align:right">
    <div><span class="live-dot"></span><span style="color:#22c55e;font-size:0.85rem">LIVE</span></div>
  ...
  <div class="charts">
    ...
    <div class="panel">
      ...
      <div class="stats-grid">
        <div class="stat-box">
          <div class="stat-val" id="statAvgTemp">--</div>
          <div class="stat-label">Avg Temp °C</div>
        </div>
        <div class="stat-box">
          <div class="stat-val" id="statAvgHum">--</div>
          <div class="stat-label">Avg Humidity %</div>
        </div>
        <div class="stat-box">
          <div class="stat-val" id="statMotionCount">--</div>
          <div class="stat-label">Motion Events</div>
        </div>
        <div class="stat-box">
          <div class="stat-val" id="statGasCount">--</div>
          <div class="stat-label">Gas Events</div>
        </div>
        <div class="stat-box">
          <div class="stat-val" id="statMaxTemp">--</div>
          <div class="stat-label">Max Temp °C</div>
        </div>
        <div class="stat-box">
          <div class="stat-val" id="statTotalRows">--</div>
...
<div class="footer">
  SentinelHome — ML IoT Group Project 2 | UTM 2025/2026 | Data via Supabase
</div>
...
async function updateLatest() {
  try {
    ...
    document.getElementById('lastUpdate').textContent = new Date(d.created_at).toLocaleTimeString();
    ...
    banner.style.display = (m || g || a || t>35 || h>80) ? 'block' : 'none';
...
async function updateHistory() {
  try {
    ...
    const motionCount = rows.filter(r=>r.motion).length;
    const gasCount = rows.filter(r=>r.gas).length;

    document.getElementById('statAvgTemp').textContent = avgTemp;
    document.getElementById('statAvgHum').textContent = avgHum;
    document.getElementById('statMaxTemp').textContent = maxTemp;
    document.getElementById('statMotionCount').textContent = motionCount;
    document.getElementById('statGasCount').textContent = gasCount;
...
if __name__ == "__main__":
    print("SentinelHome Dashboard running at http://localhost:5000")
